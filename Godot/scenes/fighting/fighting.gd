...
func _draw():
	...
	for i in range(int((- size.x) / tilesize) - 1, int((size.x) / tilesize) + 1):
		draw_line(Vector2(i * tilesize, size.y + 100) - g, Vector2(i * tilesize, -size.y-100) - g, Color.BLACK)
	for i in range(int((- size.y) / tilesize) - 1, int((size.y) / tilesize) + 1):
		draw_line(Vector2(size.x + 100, i * tilesize) - g, Vector2(- size.x - 100, i * tilesize) - g, Color.BLACK)
